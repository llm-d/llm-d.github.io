...
This page covers how to enable and interpret metrics from an llm-d deployment. For Prometheus and Grafana installation, see [Observability Setup](/docs/resources/observability/setup) first.
...
- A running llm-d deployment with an InferencePool and model servers — see the [quickstart](/docs/getting-started/quickstart) if needed
- Prometheus and Grafana installed — see [Observability Setup](/docs/resources/observability/setup)
...
The monitoring component creates PodMonitors that scrape model server metrics. See [`guides/recipes/modelserver/components/monitoring/`](https://github.com/llm-d/llm-d/tree/release-0.8/guides/recipes/modelserver/components/monitoring) for details.
