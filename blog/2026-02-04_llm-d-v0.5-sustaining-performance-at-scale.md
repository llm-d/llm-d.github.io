...
*Figure 3: Single-request KV-cache load across tiers on Llama-3.1-70B (4×NVIDIA H100). A comparison showing that while local HBM/DRAM is faster for single requests, storage offloading via a IBM Scale backed PVC provides up to 16.8x speedup over prefill for long contexts, with gains scaling by available storage bandwidth.*
