...
request's prefix each candidate endpoint holds - an endpoint being one
serving instance: often a pod, sometimes a rank within a pod or a multi-node
instance. With P2P, that knowledge becomes a transfer instruction: route the
request to the best endpoint overall, then tell it where to fetch the
missing prefix.
...
of converging on one. The main deployment uses the precise, KV-event-fed
index so the source decision reflects the blocks the engines actually
report. The GLM ablation also tests the approximate index to separate the
index and pull effects.
