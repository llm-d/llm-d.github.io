...
**The request body does not tell you the cost.** For text, the length of the body is a decent proxy for the work.
For media it tells you nothing useful. A 1280x720 image is worth roughly 880 visual tokens to Qwen3-VL — more than most text prompts — whether it arrives as a 200 KB JPEG, a 2 MB PNG, or a 60-byte URL.
...
**1. The payload introduces complex asset types and external references, not just tokenizable text. While a standard router can easily read and tokenize a "text" part, introducing multimodal elements means the router suddenly encounters non-text structures.
The heaviest and most expensive part of the request may not even be present in the payload: as seen in the multimodal example, an image_url is merely a reference, meaning the actual bytes live somewhere else.
