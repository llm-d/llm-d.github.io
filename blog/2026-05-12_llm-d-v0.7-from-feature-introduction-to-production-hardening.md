...
In our previous release (v0.6), we focused on proving the architectural bounds of what llm-d could achieve—introducing OpenTelemetry integration, prefill/decode disaggregation, and initial multi-accelerator images. While v0.6 established these capabilities, v0.7 marks the transition from a collection of promising features into rigorous production infrastructure.

Driven by over 250 pull requests, 157 commits, and contributions from 23 new developers, the theme of v0.7 is consistent across every component: removing friction, broadening hardware reach, and providing the documentation and continuous integration (CI) coverage required to trust the system in production.

Previous releases demonstrated llm-d's performance optimizations through external validations: [3× throughput gains and 2× faster TTFT with prefix-cache-aware routing](https://llm-d.ai/blog/production-grade-llm-inference-at-scale-kserve-llm-d-vllm) (Tesla/Red Hat on AMD MI300X), [40% reduction in TTFT/ITL with predicted-latency scheduling](https://llm-d.ai/blog/predicted-latency-based-scheduling-for-llms) (Google on NVIDIA GPUs), [70% higher tokens/sec with prefill/decode disaggregation](https://aws.amazon.com/blogs/machine-learning/introducing-disaggregated-inference-on-aws-powered-by-llm-d/) (AWS on B200), and [10-30% throughput improvement with disaggregated serving](https://blogs.oracle.com/ai-and-datascience/llm-inference-at-scale-with-llm-d-on-oci) (Oracle on AMD MI300X). Those capabilities remain and continue to improve, but v0.7's investment is elsewhere: making those optimizations accessible to teams who need onboarding guides, tested installation paths, and confidence the guides work on their target platform.
...
To ensure teams can onboard without tribal knowledge, v0.7 introduces 124 changed markdown files with over 10,000 lines of new content. The docs/ directory gained 72 new files, the guides directory added 164 files, and the entire guide taxonomy was restructured for clarity. What was called "inference-scheduling" became "optimized-baseline" to better reflect what it delivers. Benchmarks moved to a helpers directory, and new guides for batch gateway, flow control, and predicted latency routing fill gaps that previously required reading source code.
...
**Visual Topology**: Fifteen new SVG diagrams make system architecture legible at a glance. Each major subsystem—KV cache indexing, prefill/decode coordination, multi-tier offloading—has visual representations showing component interaction and data flow, providing the conceptual grounding operators need to reason about performance and troubleshoot issues.
...
The project is accelerating, and the foundational investments made in v0.7 set the stage for upcoming milestones. The [v0.8 release milestone](https://github.com/llm-d/llm-d/issues/1520) targets mid-June with themes spanning production readiness, reinforcement learning workloads, and expanded accelerator coverage.
