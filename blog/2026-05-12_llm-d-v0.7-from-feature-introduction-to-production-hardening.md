...
**llm-d Router**: Alongside this deployment shift, we are rebranding the core component to the **llm-d Router**. The term "inference scheduling" frequently conflicted with standard Kubernetes pod or process scheduling terminology, when the component's primary responsibility is high-level traffic routing, not cluster-level workload scheduling. Similarly, "Inference Gateway"—heavily tied to the Kubernetes Gateway API Inference Extension—is too narrow to describe all our deployment modes now that we offer these capabilities via a standalone proxy independent of a formal K8s Gateway. The llm-d Router is defined as the functional combination of a Proxy and an Endpoint Picker (EPP). This composition allows for flexible deployment: in Standalone Mode, the proxy is deployed as a sidecar container within the same Pod as the EPP; in Gateway Mode, the proxy is provisioned as a formal Kubernetes Gateway that calls into a separate EPP service. The term "llm-d Inference Gateway" remains valid but is explicitly a specific deployment pattern of the llm-d Router. The repository has been renamed accordingly to [`llm-d-router`](https://github.com/llm-d/llm-d-router).
