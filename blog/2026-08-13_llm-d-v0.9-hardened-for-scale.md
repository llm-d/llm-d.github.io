...
**Topology-aware routing** extends this with a new filter and scorer that understands the physical P/D topology for placement decisions. **CPU-tier-aware pull-source selection** distributes KV transfer load proportionally across available sources, avoiding the hot spots that emerged when all decode pods pulled from the same prefill replica. The new **offloading connector** on the disaggregation sidecar adds an initial path for KV cache transfer over standard TCP networking, expanding where P/D disaggregation can run — though RDMA remains the recommended path for production throughput.
...
**ML researchers** working on RL can follow the incubation-stage veRL and vime integrations, which bring llm-d's routing intelligence to rollout generation today — with time-slicing on the horizon for GPU-efficient training. New accelerator overlays for GB200, GB300, and expanded XPU coverage mean more hardware options with tested deployment recipes.
