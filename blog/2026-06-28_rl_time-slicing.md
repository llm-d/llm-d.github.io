...
Distributed RL post-training is a highly fragmented "stop-and-wait" loop. The pipeline operates as a continuous cycle between generation and optimization. While generation is running, optimization accelerators sit idle and vice versa.

<div style={{textAlign: 'center', margin: '20px 0'}}>
  <img src="/img/blogs/time-slicing/rl_time-slice_1.webp" alt="Figure1 : Stop-and-Wait RL loop" style={{width: '75%', height: 'auto'}} />
</div>

Traditional cloud infrastructure is designed for continuous, steady-state workloads. This alternating architecture introduces two massive systemic inefficiencies at scale:
...
This top layer encapsulates the containerized user training environment, such as a dedicated RayCluster or standalone Kubernetes pod configuration. It isolates the modeling code from infrastructure complexity:
  - **RL Loop Actor Process**: The central coordinator of the training loop. RL code imports the TimeSlice Client library, which explicitly signals phase boundaries to the control plane using simple `acquire()` and `yield()` functions.
...
1. The user's RL training loop reaches a natural pause point—such as the end of a rollout generation phase or a training step, a logical boundary where it is safe to give up the physical accelerator. At this boundary, the job invokes the TimeSlice Client library to request hardware access for its next operational phase via an `acquire()` call.
2. The TimeSlice Client dispatches an `acquire()` RPC to the TimeSlice Orchestrator, explicitly identifying the active `job_id` and the targeted group of hardware nodes it requires. This remote procedure call synchronously blocks, forcing the workload thread to wait until the platform layer determines it is safe to proceed.
3. If another independent RL job currently holds the execution lock on the designated accelerator pool, the TimeSlice Orchestrator places the incoming request into a First-In, First-Out (FIFO) queue assigned to that node group, preserving the strict order of request arrival.
4. When the active job finishes its phase and issues a `yield()` call to release access, the TimeSlice Orchestrator begins a coordinated switch. It communicates with the node-local Snapshot Agent to trigger a memory snapshot of the yielding job's active accelerator state, serializing its entire live CUDA context and moving the data off the physical accelerator into host DRAM.
5. With the physical accelerator hardware now successfully vacated and free of memory residency, the TimeSlice Orchestrator evaluates the queue state, pops the next pending job from the front of the FIFO line, and initiates the control transfer.
6. The TimeSlice Orchestrator coordinates with the destination node's Snapshot Agent a second time, executing a restore operation. The Snapshot Agent deserializes the incoming job's cached state, lifting its model weights and execution context out of host DRAM and streaming them back onto the accelerator's high-bandwidth memory (VRAM).
7. Once the physical memory restore completes successfully, the TimeSlice Orchestrator grants the execution lock to the new job and returns a success status to the blocked `acquire()` call. The worker process immediately unblocks and resumes execution exactly where it left off, requiring zero rewrites or modifications to its core modeling logic.
...
- **Co-operative Signaling Requirement**: The current architecture relies on co-operative scheduling where jobs explicitly invoke `acquire()` and `yield()` at natural phase boundaries. Ill-behaved workloads that fail to yield control cannot be preempted without aborting the active CUDA process.
...
Today, llm-d releases the Snapshot Agent along with a [well-lit path](https://github.com/llm-d-incubation/llm-d-rl-time-slicing/blob/main/guides/snapshot-agent/README.md) for integrating it into managed multi-tenant RL services to support full fine-tuning.
...
- **TimeSlice Orchestrator and TimeSlice Client**: The TimeSlice Orchestrator coordinates time-slicing across multiple jobs, managing co-operative lock queues and driving context switches at phase boundaries. Alongside it, a lightweight TimeSlice Client library will allow any RL application to integrate using a two-call API: `acquire()` before the accelerator phase and `yield()` after.
