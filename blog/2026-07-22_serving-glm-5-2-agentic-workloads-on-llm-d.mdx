...
At roughly 37-41 end-to-end output tokens per second per user, a 64-GPU llm-d deployment implies $0.05 per million input tokens and $3.55 per million output tokens at an H200 rental rate of $2.71 per GPU-hour. Z.ai's public GLM-5.2 row lists $1.40 input and $4.40 output; Claude Opus 4.8 lists $5 input and $25 output. These are sustained-load GPU-rental conversions, with prefill rental assigned to input and decode rental assigned to output, not production TCO or API-equivalent pricing.
...
* **MTP moves the frontier; prefill width recovers interactivity. With CPU offloading held constant on the 32-GPU 1P1D DEP16 topology, MTP delivers 1.6-2.0x aggregate output and 1.6-1.8x average end-to-end output per user through concurrency 128.**

* **Cost depends on how the 64 GPUs are split.** At concurrency 128, 2P2D DEP16 delivers 40.9 end-to-end output tok/s/user at $0.05/M input and $3.55/M output; 3P1D DEP16 delivers 36.6 at $0.08/M input and $1.92/M output. Both use an assumed H200 rental rate of $2.71 per GPU-hour. Choose 2P2D for lower blended cost on this input-heavy workload, or 3P1D for lower output cost and faster first token.
...
<Figure
  ...
  alt="Prefix reuse supplied by prefill and decode GPU memory through concurrency 128"
>
  DEP16 1P1D with MTP and CPU offloading. The prefill GPU hit rate stays within four percentage points of the workload's available reuse through concurrency 128. Decode-local reuse is lower because NIXL moves KV to the decode pool.
</Figure>
...
>
  Concurrency 128 with one fixed DEP16 decoder. Across 1P1D, 2P1D, and 3P1D, output rises from 3,867 to 4,770 to 4,924 tok/s, end-to-end output per user rises from 23.1 to 28.0 to 30.1 tok/s/user, and median time to first token falls from 7.9 to 4.9 to 4.0 seconds.
</Figure>
...
**Multi-Token Processing (MTP) gets more output tokens from each decode step.**
...
**At concurrency 64, median turn completion falls 34% and p90 falls 44%.**

The 32-GPU 1P1D comparison keeps CPU offloading enabled in both runs, so the measured difference comes from MTP. The configuration drafts three extra tokens per decode step and keeps KV in FP8.
...
>
  Matched 32-GPU 1P1D DEP16 deployments with CPU offloading. Each curve follows concurrency 16 through 128. MTP moves the curve right and up: at matched concurrency, aggregate output rises 1.6-2.0x and average output per user rises 1.6-1.8x. Moving right means faster output per user; moving up means more aggregate output.
</Figure>
...
The reference sweep enables native MTP and CPU KV offloading and uses disaggregated routing.

The full-input-length sweep uses max-model-len auto and keeps DEP16 across the prefill and decode pools. The 64-GPU 3P1D reference uses three two-node prefill instances and one two-node decoder across eight nodes. Between concurrency 64 and 128, median input length is about 47K-57K tokens and p99 is above 347K:
...
**Decode replicas raise output; prefill replicas shorten the wait before generation.**
...
Each optimization removes a different bottleneck. Wide expert parallelism makes room for long contexts, while routing and CPU memory keep them reusable. Separate prefill and decode pools keep long reads from disrupting generation; MTP reduces decode steps; and additional prefill replicas add capacity where the workload queues. At the same 64-GPU budget, 2P2D produces more output and finishes turns sooner, while 3P1D reaches first token sooner and restores less KV from CPU memory.
...
The deployment questions are concrete: compare P/D with a matched aggregate topology, test wider decode pools at high concurrency, and validate tool calls under MTP.
