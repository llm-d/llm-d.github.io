...
MLA and wide expert parallelism make room for repository-scale contexts in HBM. KV-cache-aware routing sends each turn to a replica that can reuse its prefix while balancing load across the deployment. CPU memory keeps evicted prefixes available. The prefill pool reads the context, the decode pool generates the response, and MTP reduces the work needed to produce it.
