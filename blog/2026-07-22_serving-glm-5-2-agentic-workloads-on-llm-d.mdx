...
<summary><strong>TL;DR: workload, measured gains, cost, and guide</strong></summary>
...
Four llm-d capabilities divide that work. Wide expert parallelism makes room for the KV cache. Locality- and load-aware routing sends each turn to the right replica. CPU memory holds prefixes that no longer fit in HBM. Separate prefill and decode pools keep reading and generation from competing for the same GPUs. MTP can then reduce the number of decode steps.

Each request follows the same path. MLA and wide expert parallelism determine how much context fits in HBM. Routing and CPU memory determine whether saved context can be reused. The prefill pool reads the context, the decode pool generates the response, and MTP accelerates generation.
...
The [appendix](#appendix-deployment-and-benchmark-details) lists the full model configuration, software stack, measured topologies, and settings for each benchmark.
