...
sidebar_label: llm-d Architecture
sidebar_position: 0
---
...
<p align="center">
    ...
    <img alt="llm-d Logo" src="https://github.com/llm-d/llm-d/raw/main/docs/assets/images/llm-d-logo.png" width="37%" />
...
Achieve SOTA Inference Performance On Any Accelerator
 ...
 [![License](https://img.shields.io/badge/License-Apache_2.0-blue.svg)](https://github.com/llm-d/llm-d/blob/main/LICENSE)
...
1. [optimized baseline](/docs/guide/Installation/optimized-baseline) - Deploy [vLLM](https://docs.vllm.ai) behind a Gateway API-based load balancer enhanced with an [inference scheduler](https://github.com/llm-d/llm-d-inference-scheduler) to decrease serving latency and increase throughput with [prefix-cache aware routing](/docs/guide/Installation/precise-prefix-cache-aware), utilization-based load balancing, fairness and prioritization for multi-tenant serving, and [predicted latency balancing (experimental)](https://github.com/llm-d/llm-d/blob/main/guides/predicted-latency-based-scheduling).
2. [Disaggregated Serving (prefill/decode disaggregation)](/docs/guide/Installation/pd-disaggregation) - Reduce time to first token (TTFT) and get more predictable time per output token (TPOT) by splitting inference into prefill servers handling prompts and decode servers handling responses, primarily on large models such as gpt-oss-120b and when processing very long prompts.
3. [Wide Expert-Parallelism](/docs/guide/Installation/wide-ep-lws) - Deploy very large Mixture-of-Experts (MoE) models like [DeepSeek-R1](https://github.com/vllm-project/vllm/issues/16037) for much higher throughput for RL and latency-insensitive workloads, using [Data Parallelism and Expert Parallelism](https://docs.vllm.ai/en/latest/serving/data_parallel_deployment.html) over fast accelerator networks.
4. [Tiered KV Prefix Caching with CPU and Storage Offload](/docs/guide/Installation/tiered-prefix-cache) - Improve prefix cache hit rate by offloading KV-cache entries to CPU memory, local SSD, and remote high-performance filesystem storage.
5. [Workload Autoscaling](/docs/guide/Installation/workload-autoscaling) - Autoscale multi-model workloads on heterogeneous shared hardware with SLO-aware cost optimization using the [Workload Variant Autoscaler](/docs/guide/Installation/workload-autoscaling/wva) or autoscale workloads on homogeneous hardware where each model scales independently using [HPA with EPP metrics](/docs/guide/Installation/workload-autoscaling/hpa-epp).

These [guides](/docs/guide) provide tested and benchmarked recipes and Helm charts to start serving quickly with best practices common to production deployments. They are extensible and customizable for particulars of your models and use cases, using standard open source components like Kubernetes, Kubernetes Gateway API, NIXL, and vLLM. Our intent is to eliminate the heavy lifting common in tuning and deploying generative AI inference on modern accelerators.

## Get Started Now

We recommend new users start with a deployment of [optimized baseline](/docs/guide/Installation/optimized-baseline).

:::note
We are currently revamping our documentation. You can also preview [our new quickstarts](https://github.com/llm-d/llm-d/blob/main/docs/wip-docs-new/getting-started/quickstart.md), which will be formally released soon.
:::
...
<p align="center">
    ...
    <source media="(prefers-color-scheme: dark)" />
    <img alt="llm-d Arch" src="https://github.com/llm-d/llm-d/raw/main/docs/assets/images/llm-d-arch.svg" width="95%" />
...
For more details of architecture see the [project proposal](https://github.com/llm-d/llm-d/blob/main/docs/proposals/llm-d.md).
...
See the [accelerator docs](https://github.com/llm-d/llm-d/blob/main/docs/accelerators/README.md) for points of contact for more details about the accelerators, networks, and configurations tested and our [roadmap](https://github.com/llm-d/llm-d/issues/146) for what is coming next.

## 🔍 Observability

- [Monitoring & Metrics](https://github.com/llm-d/llm-d/blob/main/docs/monitoring/README.md) - Prometheus, Grafana dashboards, and PromQL queries
- [Distributed Tracing](https://github.com/llm-d/llm-d/blob/main/docs/monitoring/tracing/README.md) - OpenTelemetry tracing across vLLM, routing proxy, and EPP

## 📦 Releases

Our [guides](/docs/guide) are living docs and kept current. For details about the Helm charts and component releases, visit our [GitHub Releases page](https://github.com/llm-d/llm-d/releases) to review release notes.
...
- See [our project overview](https://github.com/llm-d/llm-d/blob/main/PROJECT.md) for more details on our development process and governance.
- Review [our contributing guidelines](https://github.com/llm-d/llm-d/blob/main/CONTRIBUTING.md) for detailed information on how to contribute to the project.
- Join one of our [Special Interest Groups (SIGs)](https://github.com/llm-d/llm-d/blob/main/SIGS.md) to contribute to specific areas of the project and collaborate with domain experts.
...
This project is licensed under Apache License 2.0. See the [LICENSE file](https://github.com/llm-d/llm-d/blob/main/LICENSE) for details.

:::info Content Source
This content is automatically synced from [README.md](https://github.com/llm-d/llm-d/blob/main/README.md) on the `main` branch of the llm-d/llm-d repository.

📝 To suggest changes, please [edit the source file](https://github.com/llm-d/llm-d/edit/main/README.md) or [create an issue](https://github.com/llm-d/llm-d/issues).
:::
