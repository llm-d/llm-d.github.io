...
A specialized service that the proxy consults for every request. The [EPP](/architecture/core/router/epp) contains the routing "intelligence," using real-time signals from model servers to make optimal placement decisions.

See the [**llm-d EPP deep dive**](/architecture/core/router/epp) to learn about the the routing engine's architecture, plugin pipeline (Filters, Scorers, Pickers), and flow control mechanisms.
