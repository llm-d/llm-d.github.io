...
The llm-d architecture is built around three primary concepts: the [Router](/architecture/core/router), the [InferencePool](/architecture/core/inferencepool), and the [Model Server](/architecture/core/model-servers).

- **[llm-d Router](/architecture/core/router)** - The intelligent entry point for inference requests. It provides LLM-aware load balancing, request queuing, and policy enforcement. It is composed of two functional parts:
  ...
  - **[Endpoint Picker (EPP)](/architecture/core/router/epp)**: The routing engine that scores and selects model server pods based on real-time metrics, KV-cache affinity, and configured policies.

- **[InferencePool](/architecture/core/inferencepool)** - The API that groups Model Server pods serving the same base model via a label selector. Conceptualized as an "LLM-optimized Service", it serves as the discovery target for the Router. Additionally:
...
- **[Model Server](/architecture/core/model-servers)** - The inference engine (such as vLLM or SGLang) that executes the model on hardware accelerators (GPUs, TPUs, HPUs).

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" />
    <img alt="Basic llm-d Arch" src="/img/docs/basic-architecture.svg" />
  </picture>
</p>
...
See [KV Cache Management](/architecture/advanced/kv-management) for an overview of how these components compose.
...
<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" />
    <img alt="Advanced llm-d Arch" src="/img/docs/llm-d-arch.svg" />
  </picture>
</p>
