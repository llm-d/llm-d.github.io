...
The examples below parameterize the model as `${MODEL_NAME}` and the proxy endpoint as `${IP}`. Set `${MODEL_NAME}` to [`Qwen/Qwen3-VL-32B-Instruct`](/well-lit-paths/multimodal-serving), and set `${IP}` to the proxy endpoint IP retrieved per that guide's verification steps.
