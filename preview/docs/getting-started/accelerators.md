...
| Google | [TPU](/resources/infra-providers/gke#llm-d-on-google-kubernetes-engine-gke) | Edwin Hernandez (@Edwinhr716), Cong Liu (@liu-cong, congliu.thu@gmail.com) |
...
Google Cloud TPUs (v6e, v7) are supported when running on GKE. See the [GKE infrastructure provider docs](/resources/infra-providers/gke) for cluster setup.
...
The RDMA overlay (`modelserver/xpu/vllm-rdma/`) reuses the standard XPU vLLM base and adds one RDMA DRA claim per pod plus RDMA-specific UCX transport settings. See the [P/D Disaggregation guide](/well-lit-paths/pd-disaggregation) for deployment instructions.
