...
The [`InferenceModelRewrite`](/api-reference/inferencemodelrewrite) resource allows platform administrators and model owners to control how inference requests are routed to specific models within an InferencePool.
...
Follow the [getting started guide](/getting-started) to set up the llm-d stack.
...
This will create a deployment with the LoRA adapter environment variables pre-configured. See [`kustomization.yaml`](https://github.com/llm-d/llm-d/blob/release-0.8/docs/operations/rollouts/adapter-rollout-example/kustomization.yaml) and [`patch-lora-config.yaml`](https://github.com/llm-d/llm-d/blob/release-0.8/docs/operations/rollouts/adapter-rollout-example/patch-lora-config.yaml) for the full configuration.
...
for ((i=1; i\<=le;total_requests; i++)); do
  # 1. Send the request
